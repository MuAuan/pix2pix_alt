...
def l2_reg(weight_matrix):
    return 0.01 * K.sum(K.abs(weight_matrix)*K.abs(weight_matrix))
...
def up_conv_block_unet(x, x2, f, name, bn_axis, bn=True, dropout=False):
    ...
    x = Conv2D(f, (3,3), name=name, padding='same', kernel_regularizer=regularizers.l1_l2(0.0001))(x)
    if bn: x = BatchNormalization(axis=bn_axis)(x)
...
def up_conv_block_unet_alt(x, x2, f, name, bn_axis, bn=True, dropout=False):
    x = Activation('relu')(x)
    x = UpSampling2D(size=(2, 2))(x)
    x = Conv2D(f, (3,3), name=name, padding='same', kernel_regularizer=regularizers.l1_l2(0.0))(x)
    if bn: x = BatchNormalization(axis=bn_axis)(x)
    if dropout: x = Dropout(0.5)(x)
    #x = Concatenate(axis=bn_axis)([x, x2])
    return x
